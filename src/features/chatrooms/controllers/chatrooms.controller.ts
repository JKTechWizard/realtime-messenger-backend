...
export const chatroomsController = {
  ...
  async getById(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const room = await chatroomsService.getById(req.params.roomId as string);
  ...
  async join(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const room = await chatroomsService.join(req.user.userId, req.params.roomId as string);
  ...
  async leave(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      await chatroomsService.leave(req.user.userId, req.params.roomId as string);
